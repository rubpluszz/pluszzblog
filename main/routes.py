...
@bp.route('/read_later/<int:post_id>')
@login_required
def read_later( post_id):
    post = Post.query.filter_by(id=post_id).first_or_404()
    current_user.read_later.append(post)
    db.session.commit()
    return redirect(url_for('main.post', post_id=post_id))
...
def projects():
    selected_posts = db.session.query(Post).filter(Post.selected_posts==True).all()
    post_id = Post.query.filter_by(title='My Projects').first_or_404().id
    return redirect(url_for('main.post', post_id=post_id))

@bp.route('/section')
def section():
    selected_posts = db.session.query(Post).filter(Post.selected_posts==True).all()
    sections = Post.all_sections()
    return render_template('section.html',sections=sections, selected_posts=selected_posts)
...
def user(username, category="_favorite_posts"):
    ...
    if category=="_favorite_posts":
        data_box = user.liked_posts().paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    if category=='_recomendation':
        data_box =Post.query.filter_by(selected_posts=True).paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    if category =='_read_later':
        data_box= user.to_read_later().paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    if category =='_users' and current_user.username=='pluszz':
        data_box = User.query.paginate(page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.blog', page=data_box.next_num) if data_box.has_next else None
    prev_url = url_for('main.blog', page=data_box.prev_num) if data_box.has_prev else None    
    return render_template('user_page.html', page_to_vievs=page_to_vievs, Comments=Comments, user=user,
                           form=form ,liked_posts=data_box, prev_url=prev_url, next_url=next_url)
...
@bp.route('/dialog_by/<int:user_id>', methods=['GET', 'POST'])#inwork
@login_required
def dialog_by(user_id):
    ...
    next_url = url_for('main.messages_outbox', page=messages.next_num) if messages.has_next else None
    prev_url = url_for('main.messages_outbox', page=messages.prev_num) if messages.has_prev else None
    return render_template('dialog.html', title=_('Dialog by {}:'.forat(user.username)),
...
@bp.route('/one_sections_post/<section>')
def one_sections_posts(section):
    user = current_user
    selected_posts = db.session.query(Post).filter(Post.selected_posts==True).all()
    page = request.args.get('page', 1, type=int)
    posts = Post.query.filter_by(section=section).order_by(Post.timestamp.desc()).paginate(
        page, current_app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('main.blog', page=posts.next_num) if posts.has_next else None
    prev_url = url_for('main.blog', page=posts.prev_num) if posts.has_prev else None
    return render_template('index.html', title='PluszzBlog',
